...
const envSchema = z.object({
  ...
  MONGO_URI: z
    .string()
    .default('mongodb://127.0.0.1:27017/cold_chain?replicaSet=rs0&directConnection=true'),
  ...
  AUDIT_HMAC_KEY: z
    .string()
    .min(40, 'Run npm run setup to create a strong audit key.'),
...
export const config = {
  ...envSchema,
  origins: envSchema.APP_ORIGINS.split(',').map((s) => s.trim())
};

if (config.NODE_ENV === 'production' && config.INTEGRATION_MODE !== 'live') {
  throw new Error('Production requires live integrations.');
}
if (
  config.NODE_ENV === 'production' &&
  config.origins.some((s) => !s.startsWith('https://'))
) {
...
if (config.INTEGRATION_MODE === 'live') {
  ...
  if (
    ...
    u.search ||
    u.hash ||
    (u.protocol !== 'https:' && !(local && config.ALLOW_LOCAL_HTTP === 'true'))
  ) {
    throw new Error(
      'Use HTTPS for FHIR; local HTTP needs ALLOW_LOCAL_HTTP=true.'
    );
  }
}
