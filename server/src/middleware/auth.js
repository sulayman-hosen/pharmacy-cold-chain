import { User, Session, transaction } from '../database/models.js';
...
import {
  ...
  checkPassword,
  sameSecret
} from '../services/terminologyService.js';
import { appendAudit, audit } from '../services/auditService.js';
...
export async function login(username, password) {
  ...
  if (!u || !valid) {
    await audit({ action: 'LOGIN_FAILED', outcome: '4' });
  ...
  await transaction(async (s) => {
    await Session.create(
      [{ _id: sha256(token), userId: u._id, csrf, expiresAt }],
      { session: s }
    );
    await appendAudit(s, { actor: u._id, action: 'LOGIN_SUCCEEDED' });
  });
...
export async function authenticate(req, res, next) {
  ...
  if (!['GET', 'HEAD', 'OPTIONS'].includes(req.method))
    assert(
      sameSecret(req.get('X-CSRF-Token'), session.csrf),
      'CSRF_INVALID',
