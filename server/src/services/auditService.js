...
import { Audit, AuditHead, Outbox, transaction } from '../database/models.js';
import { config } from '../config/env.js';
import { sign } from './terminologyService.js';
...
export async function appendAudit(
  session,
  { actor = 'system', action, entityId, outcome = '0' }
) {
  ...
  const event = {
    ...
    subtype: [{ system: `${NS}/audit-action`, code: action }],
    action: action.endsWith('_READ')
      ? 'R'
    ...
    agent: [
      {
        who: { identifier: { system: `${NS}/actors`, value: actor } },
        requestor: actor !== 'system'
      }
    ],
    source: {
      observer: { identifier: { system: `${NS}/services`, value: 'coldline' } }
    },
    ...(entityId
      ? { entity: [{ what: { identifier: { system: `${NS}/indents`, value: entityId } } }] }
      : {})
  };
  const seq = head.seq + 1;
  const hash = sign(config.AUDIT_HMAC_KEY, { seq, previousHash: head.hash, event });
  await Audit.create([{ _id: id, seq, previousHash: head.hash, hash, event }], {
    session
  });
...
export async function rechainAudit() {
  return transaction(async (session) => {
    const records = await Audit.find()
      .sort({ seq: 1, recorded: 1 })
      .session(session)
      .lean();
    if (records.length === 0) {
      await AuditHead.updateOne(
        { _id: 'main' },
        { $set: { seq: 0, hash: GENESIS } },
        { session }
      );
      return {
        valid: true,
    ...
    for (const row of records) {
      ...
      const currentHash = sign(config.AUDIT_HMAC_KEY, {
        seq: currentSeq,
...
export async function deleteAndRechainAudit(targetSeq) {
  return transaction(async (session) => {
    ...
    if (filterConditions.length > 0) {
      const res = await Audit.deleteOne({ $or: filterConditions }).session(
        session
      );
