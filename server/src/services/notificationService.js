...
import { assert } from './terminologyService.js';
import { couriers } from '../fixtures.js';
import { PushSubscription } from '../database/models.js';
...
export function safeNotification(event) {
  ...
  return {
    ...
    body: `${courier.label} is on the way. ETA ${eta}. Please place in refrigerator upon receipt.`,
...
export function checkPushEndpoint(endpoint) {
  ...
  assert(
    u.protocol === 'https:' &&
      ...
      !u.port &&
      !u.hash &&
      config.PUSH_ALLOWED_HOSTS.split(',').includes(u.hostname),
    'PUSH_ENDPOINT',
    'This push service is not allowed.'
  );
}
