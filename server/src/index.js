...
import { checkFhirVersion } from './services/fhirService.js';
...
export async function start({ afterConnect, cleanup } = {}) {
  ...
  try {
    ...
    const isLocal =
      config.MONGO_URI.includes('127.0.0.1') ||
      config.MONGO_URI.includes('localhost');
    if (config.NODE_ENV === 'development' && !afterConnect && isLocal) {
      ...
      await connectDb();
      const { seed } = await import('./database/seeders.js');
      await seed();
      console.log(
        '[Coldline] In-memory MongoDB active and seeded with demo accounts.\n'
      );
    } else {
      throw err;
    }
