...
import { Button, playExcursionBeep } from '../common/UIComponents';
        ...
        {readings.map((r, i) => (
          <div key={i} className="bar-col">
            ...
            <div className="bar-bg">
              <div
                className={`bar-fill ${Number(r.temp) > 8 || Number(r.temp) < 2 ? 'fill-danger' : ''}`}
                style={{ height: `${Math.min(100, Math.max(25, (Number(r.temp) / 10) * 100))}%` }}
              />
            </div>
    ...
    <div className="qr-card">
      ...
      {!scanned ? (
        <Button kind="secondary" disabled={scanning || busy} onClick={scan}>
          <QrCode size={16} />{' '}
          {scanning
            ? t?.reticleScanning || 'Scanning camera reticle…'
            : t?.simulateScan || 'Simulate Camera Scan'}
        </Button>
      ) : (
