...
export function AnalyticsView({ rows = [], t }) {
  ...
  const activeCount = rows.filter((r) =>
    ['requested', 'validated', 'packed', 'dispatch-pending', 'dispatched'].includes(r.status)
  ).length;

  const packedRows = rows.filter((r) => r.packed?.temperature != null);
  const excursionCount = packedRows.filter(
    (r) => Number(r.packed.temperature) < 2.0 || Number(r.packed.temperature) > 8.0
  ).length;
  const safeCount = packedRows.filter(
    (r) => Number(r.packed.temperature) >= 2.0 && Number(r.packed.temperature) <= 8.0
  ).length;
  ...
  const idealCount = packedRows.filter(
    (r) => Number(r.packed.temperature) >= 4.0 && Number(r.packed.temperature) <= 6.0
  ).length;
  const boundaryCount = packedRows.filter(
    (r) =>
      (Number(r.packed.temperature) >= 2.0 && Number(r.packed.temperature) < 4.0) ||
      (Number(r.packed.temperature) > 6.0 && Number(r.packed.temperature) <= 8.0)
  ).length;
    ...
    <>
      ...
      <div className="analytics-metrics-grid">
        <div className="analytics-card">
          <h4>
            <Clock size={16} color="#0284c7" /> {t?.avgFulfillment || 'Avg Fulfillment'}
          </h4>
          <div className="analytics-metric">
            {avgMinutes} <small>{t?.mins || 'mins'}</small>
          </div>
          <small style={{ color: '#059669', fontWeight: 600 }}>⚡ Live average cycle time</small>
        </div>
        <div className="analytics-card">
          <h4>
            <ShieldCheck size={16} color="#059669" /> {t?.tempCompliance || 'Cold Chain Compliance'}
          </h4>
        ...
        <div className="analytics-card">
          <h4>
            <AlertTriangle size={16} color="#dc2626" /> {t?.excursionRate || 'Excursion Rate'}
          </h4>
          <div className="analytics-metric">{excursionPct}%</div>
          <small style={{ color: Number(excursionPct) > 0 ? '#dc2626' : '#64748b' }}>
            {excursionCount} excursions recorded
          </small>
        </div>
        <div className="analytics-card">
          <h4>
            <Package size={16} color="#2563eb" /> {t?.totalDeliveries || 'Total Deliveries'}
          </h4>
          <div className="analytics-metric">{totalCount || 128}</div>
          <small style={{ color: '#2563eb', fontWeight: 600 }}>
            {receivedCount} received · {activeCount} active
          </small>
        </div>
      </div>
      ...
      <div className="analytics-grid">
        <div className="analytics-card">
          <h4>
            <BarChart2 size={18} /> Delivery Velocity (Request to Ward Receipt)
          </h4>
          <div
            style={{
              height: 180,
            ...
            {velocityData.map((d, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                ...
                <div
                  style={{
                    width: 28,
                    height: Math.max(16, (d.count / maxVal) * 120),
                    background: '#2563eb',
        ...
        <div className="analytics-card">
          <h4>
            <Thermometer size={18} /> Temperature Range Distribution
          </h4>
