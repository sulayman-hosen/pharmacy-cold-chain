...
import { Check, FileCheck2, Package, Printer, Truck, ShieldCheck, Send } from 'lucide-react';
import { Modal, Button, Status, ErrorBox, dateTime, time } from '../common/UIComponents';
...
export function DetailModal({ item, user, config, t, onClose, onChanged }) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => api(`/indents/${row._id}`).then(d => { setRow(d); if (d.status !== 'dispatch-pending') onChanged(); }).catch(e => setError(e.message)), 2500);
  ...
  async function send() {
    ...
    try {
      ...
      setRow(d.indent); setMessage(''); onChanged();
    } catch (e) {
      setError(e.message);
    } finally {
      setBusy(false);
    ...
    <Modal wide title="Delivery workspace" subtitle={`${row.prescriptionId} · Request ${row._id.slice(0, 8)}`} onClose={onClose}>
      <div className="detail-heading">
        <div>
          ...
          <p>{row.dose} {row.unit} · {row.patientRef} · {row.floor.toUpperCase()}</p>
        </div>
        ...
        <>
          <div className="order-callout">
            ...
            <div>
              ...
              <p>Lot {row.packed.lot} · Packed {time(row.packed.at)} · Policy {row.packed.policy.minC}–{row.packed.policy.maxC}°C</p>
            </div>
            <Button kind="secondary" onClick={() => setShowPrint(true)}><Printer size={16} />{t?.printSlip || 'Print Label Manifest'}</Button>
          </div>
          <TemperatureTelemetry packedTemp={row.packed.temperature} />
        </>
      )}
      {row.dispatch && (
        <div className="order-callout" style={{ background: '#f0f9ff', borderColor: '#0284c7', flexDirection: 'column', alignItems: 'stretch', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <Truck size={26} color="#0284c7" />
            <div>
              <strong style={{ fontSize: 15, color: '#0369a1' }}>🚚 Courier Transport: {row.dispatch.courierName || config.couriers.find(c => c.id === row.dispatch.courierId)?.label || row.dispatch.courierId} (ID: {row.dispatch.courierId})</strong>
              <p style={{ margin: '2px 0 0', fontSize: 13, color: '#334155' }}><strong>Departed:</strong> {time(row.dispatch.departedAt)} · <strong>ETA:</strong> {time(row.dispatch.eta)} · <strong>Dispatch Temp:</strong> {row.dispatch.temperature}°C</p>
            </div>
          </div>
          <div style={{ fontSize: 12, color: '#0369a1', background: '#e0f2fe', padding: 8, borderRadius: 6 }}>
            {row.status === 'dispatch-pending' ? 'Departure accepted. The EHR chart sync is queued; nurse push alert follows successful write.' : 'The departure has been recorded in the patient chart and delivered by courier.'}
          </div>
        </div>
      )}
      {pharmacist && row.status === 'requested' && (
        <div className="action-area">
          ...
          <Button disabled={busy} onClick={() => action('validate')}><ShieldCheck size={17} />Approve request</Button>
        </div>
      )}
      {pharmacist && row.status === 'validated' && (
        <form className="action-area" onSubmit={e => { e.preventDefault(); action('pack', { ...pack, temperature: Number(pack.temperature), expiresAt: new Date(pack.expiresAt + 'T23:59:59Z').toISOString() }); }}>
          <h3>Pack & record the cold chain</h3>
          <div className="form-grid three">
            <label>Temperature (°C)<input required type="number" step="0.1" value={pack.temperature} onChange={e => setPack({ ...pack, temperature: e.target.value })} /></label>
            <label>Batch / lot<input required value={pack.lot} onChange={e => setPack({ ...pack, lot: e.target.value })} /></label>
            <label>Expiry date<input required type="date" value={pack.expiresAt} onChange={e => setPack({ ...pack, expiresAt: e.target.value })} /></label>
          </div>
          <Button disabled={busy}><Package size={17} />Confirm packing</Button>
        </form>
      )}
      {pharmacist && row.status === 'packed' && (
        <div className="action-area">
          ...
          {config.integrationMode === 'demo' && (
            <>
              <div className="form-grid three">
                <label>Courier Name & ID
                  <select value={delivery.courierId} onChange={e => updateDelivery({ ...delivery, courierId: e.target.value })}>
                    {config.couriers.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
                  </select>
                </label>
                <label>ETA in minutes
                  <input type="number" min="1" max="240" value={delivery.minutes} onChange={e => updateDelivery({ ...delivery, minutes: Number(e.target.value) })} />
                </label>
                <label>Departure °C
                  <input type="number" step="0.1" value={delivery.temperature} onChange={e => updateDelivery({ ...delivery, temperature: Number(e.target.value) })} />
                </label>
              </div>
              <Button kind="secondary" disabled={busy} onClick={generate}><Send size={16} />Generate demo HL7</Button>
            </>
          )}
          <label className="top-gap">HL7 message
            <textarea className="code-input" rows="7" placeholder="MSH|^~\&|..." value={message} onChange={e => setMessage(e.target.value)} />
          </label>
          <Button disabled={busy || !message.trim()} onClick={send}><Send size={16} />Process departure</Button>
        </div>
      )}
      ...
      <div className="modal-actions split">
        <Button kind="ghost" disabled={busy} onClick={showOrder}><FileCheck2 size={16} />View FHIR prescription</Button>
        {['requested', 'validated', 'packed'].includes(row.status) && (
          <Button kind="danger" disabled={busy} onClick={() => action('cancel')}>Cancel request</Button>
        )}
      </div>
      {resource && (
