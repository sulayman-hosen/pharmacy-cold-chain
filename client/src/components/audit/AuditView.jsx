...
export function AuditView({ t }) {
  ...
  async function deleteTamperedRecord(seqOrId) {
    const target =
      seqOrId ||
      (selectedAudit?._id || selectedAudit?.seq) ||
      (verification?.failedRecord?._id || verification?.failedAt || 1);
  ...
  function openFailedRecordModal() {
    ...
    const targetId = verification?.failedRecord?._id;
    const failedItem =
      verification?.failedRecord || rows.find((r) => r.seq === targetSeq || (targetId && r._id === targetId)) || rows[0];
    if (failedItem) {
  ...
  function renderEventBadge(code) {
    ...
    if (codeUpper === 'INDENT_CREATED')
      return (
        <span className="badge status-dispatched" style={{ gap: 4 }}>
          <Plus size={12} /> {formatted}
        </span>
      );
    if (codeUpper === 'INDENT_VALIDATED')
      return (
        <span className="badge status-validated" style={{ gap: 4 }}>
          <ShieldCheck size={12} /> {formatted}
        </span>
      );
    if (codeUpper === 'MEDICATION_PACKED')
      return (
        <span className="badge status-requested" style={{ gap: 4 }}>
          <Package size={12} /> {formatted}
        </span>
      );
    if (codeUpper === 'COURIER_DEPARTED')
      return (
        <span className="badge status-dispatch-pending" style={{ gap: 4 }}>
          <Truck size={12} /> {formatted}
        </span>
      );
    if (codeUpper === 'DELIVERY_RECEIVED')
      return (
        <span className="badge status-received" style={{ gap: 4 }}>
          <CheckCircle2 size={12} /> {formatted}
        </span>
      );
    if (codeUpper.includes('READ'))
      return (
        <span className="badge" style={{ background: '#f1f5f9', color: '#475569', gap: 4 }}>
          <FileCheck2 size={12} /> {formatted}
        </span>
      );
    return (
    ...
    <>
      <div className="page-title">
        ...
        <div className="title-actions">
          <Button kind="secondary" onClick={() => setShowCertModal(true)}>
            <Printer size={16} /> Print Certificate
          </Button>
          <Button kind="secondary" onClick={downloadCsv}>
            <Download size={16} />
            {t.exportCsv || 'Export CSV Report'}
          </Button>
          <Button kind="secondary" onClick={verifyChain} disabled={isVerifying}>
            <ShieldCheck size={17} />
            {isVerifying ? 'Verifying...' : t.verifyChain || 'Verify Audit Chain'}
          </Button>
        </div>
      </div>
      ...
      <div className="analytics-metrics-grid" style={{ marginBottom: 20 }}>
        <div className="analytics-card" style={{ borderLeft: '4px solid #059669' }}>
          <h4 style={{ color: '#047857' }}>
            <LockKeyhole size={16} color="#059669" /> 1. HIPAA Data Minimization
          </h4>
          <div className="analytics-metric" style={{ fontSize: 22, color: '#047857' }}>
            100% Compliant
          </div>
        ...
        <div
          ...
          style={{ borderLeft: `4px solid ${verification?.valid === false ? '#dc2626' : '#0284c7'}` }}
        >
          ...
          <div
            ...
            style={{ fontSize: 22, color: verification?.valid === false ? '#b91c1c' : '#0369a1' }}
          >
            {verification
              ? verification.valid
                ? 'Chain Intact'
                : `Tamper Alert! (#${verification.failedAt || 1})`
              : 'HMAC Verified'}
          </div>
          <small style={{ color: verification?.valid === false ? '#b91c1c' : '#0369a1', fontWeight: 600 }}>
            {verification
              ? verification.valid
                ? `${verification.checked} records signed with HMAC-SHA256`
                : `Mismatch at record #${verification.failedAt || 1}`
              : 'HMAC-SHA256 Sequence Chaining Active'}
          </small>
        </div>

        <div className="analytics-card" style={{ borderLeft: '4px solid #7c3aed' }}>
          <h4 style={{ color: '#6d28d9' }}>
            <Activity size={16} color="#7c3aed" /> 3. Chain of Custody & Clinical
          </h4>
          <div className="analytics-metric" style={{ fontSize: 22, color: '#6d28d9' }}>
            NIH RxNav Verified
          </div>
          <small style={{ color: '#7c3aed', fontWeight: 600 }}>✓ RxNorm Concept & 2°C–8°C Transport Policy Enforced.</small>
        </div>
      </div>
      ...
      <section className="panel" style={{ marginBottom: 20 }}>
        <div className="panel-top">
          <div>
            ...
            <p>
              HIPAA Data Minimization Rule: Summary views expose Patient Reference IDs & Ward Locations only. Full FHIR Resources
              linked within AuditEvents.
            </p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14, padding: 16, background: '#f8fafc', borderRadius: 8 }}>
            <div style={{ background: '#ffffff', padding: 12, borderRadius: 6, border: '1px solid #e2e8f0' }}>
              ...
              <strong style={{ fontSize: 14, color: '#0f172a' }}>
                {lifecycleData?.indentSource?.nurseId
                  ? `${lifecycleData.indentSource.nurseId} / ${lifecycleData.indentSource.ward}`
                  : 'Floor Nurse / Ward IPD-3'}
              </strong>
            ...
            <div style={{ background: '#ffffff', padding: 12, borderRadius: 6, border: '1px solid #e2e8f0' }}>
              ...
              <strong style={{ fontSize: 14, color: '#7c3aed' }}>
                RxNorm: {lifecycleData?.fulfillment?.rxNormCode || '274783'} (
                {lifecycleData?.fulfillment?.drugName ? lifecycleData.fulfillment.drugName.split(' ')[0] : 'NIH RxNav'})
              </strong>
              <div style={{ fontSize: 12, color: '#334155', marginTop: 4 }}>
      ...
      {verification && (
        <div
          className={`verification ${verification.valid ? 'valid' : 'invalid'}`}
          style={{ marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, minWidth: 280 }}>
            ...
            <div>
              <strong style={{ fontSize: 15 }}>
                {verification.valid
                  ? t.auditVerified || 'HMAC Cryptographic Audit Chain Verified'
                  : `Audit Chain Sequence Mismatch Detected at record #${verification.failedAt || 1}`}
              </strong>
              <p style={{ marginTop: 4 }}>
                {verification.valid
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <span className="small-tag" style={{ background: '#0f172a', color: '#ffffff' }}>
              HMAC-SHA256
            </span>
          </div>
        </div>
      ...
      {subTab === 'events' && (
        <section className="panel">
          <div className="panel-top">
            <div>
              <h3>
                FHIR AuditEvent Legal Logs <span className="count">{filteredRows.length}</span>
              </h3>
          ...
          <div className="table-tools" style={{ padding: '12px 16px', display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
            <div className="search" style={{ flex: 1, minWidth: 200 }}>
              <Search size={16} />
              <input placeholder="Search by event, actor ID, or hash..." value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
            ...
            >
              <div>
                ...
                <p style={{ margin: '4px 0 0', fontSize: 12, color: '#991b1b' }}>
                  {verification.reason ||
                    `Expected record #${verification.failedAt || 1}, but it was deleted directly. Click Auto-Repair to re-index.`}
                </p>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  className="button secondary"
                  style={{ background: '#dc2626', color: '#fff', border: 0, fontSize: 12 }}
                  onClick={() => deleteTamperedRecord(verification.failedAt || 1)}
                >
                ...
                <button
                  className="button secondary"
                  style={{ background: '#0284c7', color: '#fff', border: 0, fontSize: 12 }}
                  onClick={repairChain}
                >
                  🛠️ Auto-Repair Chain
                </button>
                  ...
                  return (
                    ...
                    >
                      <td className="mono" style={{ fontWeight: 700 }}>
                        ...
                        {isMismatched && (
                          <span style={{ display: 'block', fontSize: 10, color: '#dc2626', fontWeight: 800 }}>MISMATCH</span>
                        )}
                      </td>
                      <td>
