...
import { 
  LayoutDashboard, Package, Truck, Bell, ShieldCheck, Plug, ArrowRight, Plus, 
  RefreshCw, LogOut, ChevronRight, FileCheck2, LockKeyhole, Activity, 
  ClipboardList, CheckCircle2, Sun, Moon, Globe, BarChart2 
} from 'lucide-react';
import { api, setCsrf, enablePush } from './api';
import { translations } from './translations';
import { Mark, Button, ErrorBox, Empty, dateTime } from './components/common/UIComponents';
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand">
          <Mark />
          <span>COLDLINE<span className="brand-period">.</span></span>
        </div>
      ...
      <div className="main-column">
        <header className="topbar">
          ...
          <div className="topbar-right">
            ...
            <button className="icon-button theme-toggle" title="Toggle Theme" onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')}>
        ...
        <main>
          ...
          {(tab === 'overview' || tab === 'requests') && (
            <>
              ...
              {tab === 'overview' && (
                <>
                    ...
                    {[
                      ...
                      <div className="metric" key={title}>
                        <div className="metric-heading">
                          <span>{title}</span>
                          <span className={`metric-icon ${color}`}><Icon size={17} /></span>
                        </div>
                  ...
                  <div className="care-banner">
                    ...
                    <span className="banner-pill"><ShieldCheck size={14} /> {t.controlledHandoffs}</span>
                  </div>
                </>
              )}
                ...
                {tab === 'overview' && (
                  <aside className="right-rail">
                    ...
                    <section className="privacy-card">
                      <span className="privacy-icon"><LockKeyhole size={21} /></span>
                      <h3>{t.privateByDesign}</h3>
                      <p>{t.privateDesc}</p>
                      <div className="notification-preview">
                        <div><strong>COLDLINE</strong><small>Preview</small></div>
                        <b>Delivery update</b>
                        <p>Courier 07 is on the way.<br />Open Coldline for details.</p>
                      </div>
                    </section>
                  </aside>
                )}
              ...
              <p className="footnote">
                {config.integrationMode === 'demo' ? 'Demo data and terminology fixtures · No real patient information' : 'Strict formulation matching · No automatic medication substitutions'}
          ...
          {tab === 'inbox' && (
            <>
                ...
                {notifications.length ? notifications.map(n => {
                  ...
                  return (
                    <article key={n._id} className={n.readAt ? 'read' : ''}>
                      <span className="inbox-icon"><Truck size={22} /></span>
                      <div>
                        <h3>{displayTitle}{!n.readAt && <span className="unread-dot" />}</h3>
                        <p>{displayBody}</p>
              ...
              {!config.pushEnabled && (
                <p className="footnote">
                  {t.inappFootnote || "In-app alerts are active. Add VAPID credentials in the server configuration to enable real Web Push."}
                </p>
              )}
            </>
          )}
        ...
        <footer className="app-footer">
          <span>COLDLINE. <span>Care in every connection.</span></span>
      ...
      {newRequest && (
        <RequestModal 
          ...
          onCreated={() => { setNew(false); refresh(); }} 
        />
      )}
      {selected && (
        <DetailModal 
